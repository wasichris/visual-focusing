...
```
向上：target.bounds.y + target.bounds.height <= current.bounds.y
      （目標視窗底部 ≤ 當前視窗頂部）

向下：target.bounds.y >= current.bounds.y + current.bounds.height
      （目標視窗頂部 ≥ 當前視窗底部）

向左：target.bounds.x + target.bounds.width <= current.bounds.x
      （目標視窗右邊 ≤ 當前視窗左邊）

向右：target.bounds.x >= current.bounds.x + current.bounds.width
      （目標視窗左邊 ≥ 當前視窗右邊）
```
...
const verticalDistance = Math.abs(targetCenterY - currentCenterY);

// 水平距離
if (有重疊) {
  horizontalDistance = 0;  // 重疊時水平距離為 0
} else {
  // 計算中心點水平距離
  const currentCenterX = current.x + current.width / 2;
  const targetCenterX = target.x + target.width / 2;
  horizontalDistance = Math.abs(targetCenterX - currentCenterX);
}

// 總距離
distance = Math.sqrt(verticalDistance² + horizontalDistance²);
```
...
const currentCenterX = current.x + current.width / 2;
const targetCenterX = target.x + target.width / 2;
const horizontalDistance = Math.abs(targetCenterX - currentCenterX);

// 垂直距離
if (有重疊) {
  verticalDistance = 0;  // 重疊時垂直距離為 0
} else {
  const currentCenterY = current.y + current.height / 2;
  const targetCenterY = target.y + target.height / 2;
  verticalDistance = Math.abs(targetCenterY - currentCenterY);
}
...
**重疊時的特殊處理**：
- 有重疊時，非主方向的距離設為 0
- 例如向上搜尋時，有重疊的視窗 `horizontalDistance = 0`
- 這確保重疊視窗的評分更高
...
|------|--------|------|---------|---------|------|
| A | 80% | 100px | 2 | 400 - 100 - 20 | **280** ✓ 最高分 |
| B | 100% | 350px | 0 | 500 - 350 - 0 | **150** |
| C | 50% | 50px | 1 | 250 - 50 - 10 | **190** |
...
【重疊組】候選視窗: 1 個
  1. Window1 [Z:1] [重疊✓]
     加權分數:350 = 可見×500(400) - 垂直距離(30) - Z懲罰(20)
     可見面積比例: 80.0%

【不重疊組】候選視窗: 1 個
  1. Window2 [Z:3]
     加權分數:120 = 可見×500(200) - 垂直距離(50) - Z懲罰(30)
     可見面積比例: 40.0%
